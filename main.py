...
def parse_hx_category():
    ...
    if res.status_code == 200:
        ...
        #获取搜索页面链接
        selector = bs.select('#header > div > div.bd2 > div.bd3 > div:nth-child(2) > div:nth-child(1) > div > div.search > form > 

div.searchl > p:nth-child(1) > select')
        for item in selector:
            for child in item.children:
                if type(child) == bs4.element.Tag:
...
class hxplugin(StellarPlayer.IStellarPlayerPlugin):
    ...
    def makeLayout(self):
        ...
        controls = [
            ...
            {'type':'space','height':10},
            {'type':'list','name':'list','itemlayout':

{'group':list_layout},'value':self.movies,'separator':True,'itemheight':40},
            {'group':
                [
                    {'type':'space'},
                    {'group':
...
放','width':30,'@click':'onPlayClick'},{'type':'space'}]}
                controls = 

{'type':'list','name':'list','itemlayout':list_layout,'value':self.search_movies,'separator':True,'itemheight':40}
